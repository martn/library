 ...
 */
final class RequestButtonHelper extends NObject
{
	 ...
	 * @return Link|string
	 */
	static public function prepareAction(NAppForm $form, $url)
	{
	 ...
	 * @return array
	 */
	static public function prepareHttpData(NAppForm $form, $data)
	{
	  if (!$data)
	  {
			$receivedId = $form->presenter->getParam(RequestButton::RECEIVED_KEY);
			if ($receivedId AND $form->lookupPath('Nette\Application\NPresenter', TRUE) === RequestButtonStorage::getFormName($receivedId) AND RequestButtonStorage::is($receivedId))
			{
	...
	{
		if ($form === NULL) $presenter = NEnvironment::getApplication()->getPresenter();
	 ...
	 *
	 */
	static public function prepareForm(NAppForm $form)
	{
	 ...
	 */
	static public function addRequestButton(NFormContainer $form, $name, $caption = NULL, $destination = NULL, $destinationArgs = array())
	{
	 ...
	 */
	static public function addRequestButtonBack(NFormContainer $form, $name, $caption = NULL)
	{
