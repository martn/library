...
/*
TODO: kdyz je RequestButton prvni tak ho enterem odeslu (viz listopadova PS)
 ...
 */
class RequestButton extends NSubmitButton
{
	...
	public function __construct($caption = NULL, $destination = NULL, $destinationArgs = array())
	{
		...
		$this->monitor('Nette\Forms\NForm');
		$this->monitor('Nette\Application\NPresenter');
	...
	protected function attached($parent)
	{
		if ($parent instanceof IPresenter)
		{
			...
			if ($receivedId AND !($this->lookup('Nette\Forms\NForm') instanceof RequestButtonReceiver) AND $this->lookupPath('Nette\Application\NPresenter', TRUE) === RequestButtonStorage::getName($receivedId) AND RequestButtonStorage::is($receivedId))
			{
	...
	public function saveRequstAndRedirect()
	{
	  $key = RequestButtonStorage::save(
			...
			$this->lookupPath('Nette\Application\NPresenter', TRUE),
			$this->form->lookupPath('Nette\Application\NPresenter', TRUE),
			$this->form->getValues()
		);
