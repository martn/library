...
class DibiPostgreDriver extends DibiObject implements IDibiDriver
{
	...
	public function fetch($assoc)
	{
		...
		{
			if(pg_field_type($this->resultSet, $i) == 'bool') array_push($bools, array($i, pg_field_name($this->resultSet, $i)));
		}
		
		$arr = pg_fetch_array($this->resultSet, NULL, $assoc ? PGSQL_ASSOC : PGSQL_NUM);
			
		
		if(is_array($arr)) // could be false....
			foreach($bools as $bool)
			{
				
				$index = $assoc ? $bool[1] : $bool[0];
				
				if(!is_numeric($arr[$index]) && !is_bool($arr[$index]))
				{
					$arr[$index] = strtolower($arr[$index]) == "f" ? 0 : 1;  
				}  		
