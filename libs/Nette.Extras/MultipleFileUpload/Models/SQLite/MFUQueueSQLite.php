...
class MFUQueueSQLite extends MFUBaseQueueModel {
	 ...
	 */
	function addFile(NHttpUploadedFile $file) {
	...
	function getUploadedFilesTemporaryPath() {
		if(!MFUQueuesSQLite::$uploadsTempDir) {
			MFUQueuesSQLite::$uploadsTempDir = NEnvironment::expand("%tempDir%".DIRECTORY_SEPARATOR."uploads-MFU");
		}
		...
		if(!is_writable(MFUQueuesSQLite::$uploadsTempDir)) {
			MFUQueuesSQLite::$uploadsTempDir = NEnvironment::expand("%tempDir%");
		}
