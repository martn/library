...
// <editor-fold defaultstate="collapsed" desc="variables">
	...
	public static $disabledItemTitle = "x x x x x x x x";
	...
	public function __construct($label, $parents, $dataCallback) {
		...
		foreach($parents as $parent) {
			...
			if($parent instanceof DependentSelectBox)
				$parent->childs[] = $this;
			if(!self::$disableChilds && $this->isRoot($parent) && $parent instanceof NSelectBox && $parent->isFirstSkipped())
		...
		if($this->autoSelectRootFirstItem) {
			foreach($this->parents as $parent) {
				if($this->isRoot($parent) && $parent->getValue() === null) {
					if(!($parent instanceof NSelectBox))
